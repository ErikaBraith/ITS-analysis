...
** do-file: cr-its-grdata
** Interrupted time series analysis - August 2017
** Unlike the analysis file - none of the variables are temporary
** Goal is to export everything into R for plotting
...
* export everything to R 

export delimit year pnums bothsexes omenums using its-grdata, replace
...
* the issue is = 
what gets bootstrapped is the overall difference in the post-policy period
mean(omenums). 
But what I'd like to try to do is bootstrap the annual differences... 
Is that possible?
...
gen `omenums' = bothsexes - `pnums`g'' // obs - exp events after ADAA
	qui sum `omenums' if adaa==1 
